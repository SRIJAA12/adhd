...
// Ambient Sound with External URL - No local files needed!
...
// Notification Sound (optional - can use browser's default)
const notificationSound = new Howl({
  src: ['https://www.soundhelix.com/examples/mp3/SoundHelix-Song-2.mp3'],
  volume: 0.5,
});

// Urgency Bar Component with Color Transitions (Green → Yellow → Red)
function UrgencyBar({ intervalCount, workDuration }) {
  ...
  // Color logic: Green (>50%) → Yellow (25-50%) → Red (<25%)
...
export default function PomodoroTimer() {
  ...
  // Ambient Sound Control - Works with external URL!
  useEffect(() => {
    if (soundPlaying) {
      ambientSound.play();
    } else {
